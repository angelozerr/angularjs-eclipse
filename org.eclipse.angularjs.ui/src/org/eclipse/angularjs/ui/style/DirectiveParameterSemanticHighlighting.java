...
public class DirectiveParameterSemanticHighlighting extends
		...
		if (isDirectiveElement(node, file)) {
			...
			if (attributes != null) {
				...
				for (int i = 0; i < attributes.getLength(); i++) {
					...
					if (isDirectiveParameter(currentNode, file)) {
	...
	private boolean isDirectiveElement(IDOMNode node, IFile file) {
		...
		return AngularDOMUtils.getAngularDirective(
				file != null ? file.getProject() : null, (IDOMElement) node) != null;
	}

	protected boolean isDirectiveParameter(IDOMNode node, IFile file) {
		if (node.getNodeType() != Node.ATTRIBUTE_NODE) {
			return false;
		}
		return AngularDOMUtils.getAngularDirectiveParameter(
				file != null ? file.getProject() : null, (IDOMAttr) node) != null;
	}
